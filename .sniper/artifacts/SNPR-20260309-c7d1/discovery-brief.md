...
The following open questions were resolved with the user:

1. **Framework:** React 19 + Vite
2. **Animation library:** Motion (formerly Framer Motion)
3. **State management:** Zustand
4. **Mobile priority:** Desktop-first, responsive down to tablet. Phones are functional but not the primary target.
5. **Styling approach:** TBD by architect (CSS Modules used in v1)
6. **Testing strategy:** Engine unit tests only, using Vitest
7. **Game-over experience:** TBD by architect (Motion enables celebration animations)
8. **Undo/redo:** No — New Game button only
9. **Dark mode:** Dark mode only (no light/dark toggle)
10. **Deployment:** None — local dev only (`npm run dev`)
